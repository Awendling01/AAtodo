...
export const reducer = (state: AAState, action: ReducerAction) => {
  const isTodoListAction = (action: any): action is TodoListAction => {
    return action.payload.incomplete !== undefined;
  };
  const isTodoAction = (action: any): action is TodoAction => {
    return action.payload.sortOrder !== undefined;
  };
        ...
        if (completed) {
          return {
            ...
            todos: {
              ...state.todos,
              completed: [
                ...state.todos.completed.filter((td) => td.id !== id),
              ],
            },
          };
        } else {
  ...
  } else if (isTodoListAction(action)) {
    switch (action.type) {
